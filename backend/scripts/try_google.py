"""End-to-end smoke test for the Google AI Studio client.

Hits Gemini with a minimal prompt + one fake tool and prints every stream
event (text chunks, tool calls, usage). No DB, no session.

Usage:
  .venv/bin/python -m scripts.try_google
  .venv/bin/python -m scripts.try_google gemini-2.5-flash  # override model
"""
...
from app.core.google_client import stream_message
...
async def main() -> None:
    ...
    model = sys.argv[1] if len(sys.argv) > 1 else settings.groq_heavy_model
